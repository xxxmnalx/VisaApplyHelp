...
export function generateStaticParams() {
  return listGuides("KR").map((g) => ({ slug: g.slug }));
}

export default function KrGuidePage({ params }: { params: { slug: string } }) {
  const raw = getGuideSource("KR", params.slug);
  if (!raw) notFound();
  ...
  return (
    <main className="mx-auto max-w-md px-4 py-10">
      ...
      <p className="mt-8 text-xs text-neutral-500">
        <Link href="/kr" className="underline">
          ← 返回韩国列表
        </Link>
      </p>
    </main>
